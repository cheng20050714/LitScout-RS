...
3. 并发抓取 GitHub 与 arXiv；显式启用时额外抓取 Stage A 学术源。
4. canonical merge、排序、分类、EvidenceQualityGate 精选入库和 CitationLedger 构建。
5. DeepSeek 基于结构化可引用来源生成中文分析。
6. 写入 Markdown 报告和 session JSON。

旧端点 `/api/plan`、`/api/run/stream` 和 `/api/report/translate` 保持兼容；报告级 `/api/report/chat*` 已删除，追问能力收口到阅读库单篇论文页。
...
- `agent/academic_scout.rs`：在 `academic_extra_enabled` 开启时按固定 registry 执行 Semantic Scholar / DBLP / OpenAlex / Crossref Stage A 扩源。
- `agent/evidence.rs`：构建 canonical evidence pool、EvidenceMemory 和 query lineage。
- `agent/evidence_quality.rs`：对扩展学术源执行确定性精选入库，过滤低信息、无主题命中或语义歧义候选。
...
- 阅读库：基于单篇论文生成深度阅读笔记，并在论文页进行流式追问。

## 安全边界

- LLM 不允许自行联网。
- LLM 不允许引用 CitationLedger / EvidenceMemory 之外的 URL。
- Stage A 扩源默认关闭；只有 CLI `--academic-extra` 或 Web policy 显式开启时才调用 Semantic Scholar / DBLP / OpenAlex / Crossref。
- 扩展学术源只作为结构化学术来源进入统一证据流，不引入开放 Web 搜索。
- DBLP / Crossref 记录按“书目元数据”弱证据处理；扩展源候选必须通过 EvidenceQualityGate 后才进入 EvidenceMemory 和 CitationLedger。
