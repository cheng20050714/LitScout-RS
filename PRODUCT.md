...
LitScout-RS 把中文调研主题转换为可检查的研究计划，抓取 GitHub 和 arXiv 资料，生成带引用的中文报告，并支持把 arXiv 论文加入阅读库做全文笔记和单篇追问。

当前版本是 GitHub + arXiv 双源研究侦察工作台。后续搜索边界扩展会优先加入多学术索引和结构化来源，例如 Semantic Scholar、DBLP、OpenAlex、Crossref、ACL Anthology 和 OpenReview，再考虑辅助 Web 证据层；开放网页不会成为默认核心来源。扩源路线详见 `SEARCH_BOUNDARY_EXPANSION_PLAN.md`。
