...
中文调研主题
  ...
  -> 可选：Semantic Scholar / DBLP / OpenAlex / Crossref 执行 Stage A 学术扩源
  -> EvidenceBuilder 做 canonical merge、排序、分类和 EvidenceQualityGate 精选入库
  -> 构建 EvidenceMemory、CitationLedger 和 query lineage
  ...
  -> 可选：把 arXiv 论文加入阅读库做单篇论文追问
```

LLM 只能分析 LitScout-RS 已抓取到的 GitHub/arXiv 数据，以及显式启用后的结构化学术扩源数据，不允许自行联网、编造来源或修改原始引用。
...
# 可选；开启 --academic-extra 时用于 Stage A 学术扩源
...
2. Agent 阶段：输入中文 prompt，创建 stateful run，审查 Plan Tree，批准后启动 GitHub/arXiv 调研；可显式开启 Stage A 学术扩源，查看状态机进度、证据、覆盖度、引用审计和报告。

当前 Web 能力：

- 中文 ResearchBrief、ChapterPlan 与 QueryPortfolio 生成。
- Plan Tree 可编辑并保存为新的 PlanReady checkpoint。
- GitHub/arXiv 抓取进度通过 SSE 事件展示；开启扩展学术源后会额外抓取 Semantic Scholar、DBLP、OpenAlex、Crossref。
- EvidenceMemory 按章节展示证据、来源链接、query lineage 和扩展源精选摘要。
...
旧的 `POST /api/runs/:run_id/follow-up` 和报告级 `POST /api/report/chat*` 已删除；调研页不再保留第二阶段“追问”链路。
...
- 默认数据源仍限定为 GitHub 和 arXiv；`--academic-extra` 或 Web policy 可显式启用 Semantic Scholar、DBLP、OpenAlex 与 Crossref。
- 扩展学术源候选会先经过 EvidenceQualityGate，只有通过主题相关性和可验证元数据筛选的条目会进入 EvidenceMemory 和 CitationLedger。
...
- 单篇论文追问接口是 SSE 流式形态；当前实现是后端获得完整回答后按 Markdown 段落推送，后续可替换为 DeepSeek 原生 token streaming。
